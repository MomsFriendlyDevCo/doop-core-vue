...
module.exports = function DoopFrontendVue(options) {
	...
	return Promise.all([
		...
		// Kludge - rewrite braindead vue-loader code that assumes every function returns an exportable object to actually do that {{{
		.then(()=> fs.promises.readFile(`${app.config.paths.root}/dist/app.main.js`, 'utf8'))
		.then(contents => contents.replace(/(var options = typeof scriptExports === 'function'.*? : scriptExports)/g, '$1 || {}'))
		.then(contents => fs.promises.writeFile(`${app.config.paths.root}/dist/app.main.js`, contents))
		// }}}
